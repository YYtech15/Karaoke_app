...
const HomeAppBar = () => {
    ...
    const movePage = (event: React.MouseEvent<HTMLElement>) => {
        const next_page = event.currentTarget.textContent;
        if (next_page === "プロフィール") {
            navigate("/profile");
                    ...
                    <Box sx={{ flexGrow: 0 }}>
                        <Tooltip title="Open settings">
                            <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                                <Avatar alt="YY" src="/static/images/waiwai.jpg" />
                            </IconButton>
                        </Tooltip>
                        <Menu
                            sx={{ mt: '45px' }}
                        ...
                        >
                            {settings.map((setting) => (
                                <MenuItem key={setting} onClick={movePage}>
